...
// // ─── Abstract API live check (calls our /api/validate-email proxy) ────────────
 
// type EmailCheckStatus = "idle" | "checking" | "valid" | "invalid";
 
// async function checkEmailWithAbstract(email: string): Promise<{ valid: boolean; reason?: string }> {
//   const res = await fetch(`/api/validateEmail?email=${encodeURIComponent(email)}`);
//   if (!res.ok) return { valid: true }; // fail open so real users aren't blocked
//   return res.json();
// }
...
//   // ── Email blur → format check → Abstract API ──────────────────────────────
...
//     try {
//       const result = await checkEmailWithAbstract(formData.email);
...
//   // ── Debounced Abstract check while typing (after first blur) ──────────────
...
//           setEmailCheckStatus("checking");
//           try {
//             const result = await checkEmailWithAbstract(value);
//             setEmailCheckStatus(result.valid ? "valid" : "invalid");
//             setErrors((p) => ({
//               ...p,
